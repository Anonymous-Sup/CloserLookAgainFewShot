...
class TUBerlin(BaseImageDataset):
    ...
    def __init__(self, config=None, root='', verbose=True, pid_begin=0, **kwargs):
        ...
        self.base_label2index, self.novel_label2index, self.selected_label2inds = buildLabelIndex(Way=config.FEWSHOT.NWAY, seed=0)
        ...
        train, val, query, gallery = self._process_dir(self.rgb_dir, self.sketch_dir, relabel=False, 
                                                  training_mode=self.training_mode, number_pthots=config.FEWSHOT.KSHOT, number_sketches=config.FEWSHOT.KSHOT, random_seed=1)
        if verbose:
